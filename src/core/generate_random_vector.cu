...
__global__ void generate_complex_normals_v(curandState *states, cuDoubleComplex* out, int num_elements) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= num_elements) return;
    curandState localState = states[idx];
    double real = curand_normal_double(&localState);
    double imag = curand_normal_double(&localState);
    out[idx] = make_cuDoubleComplex(real, imag);
    states[idx] = localState;
}

__device__ double complex_abs_squared(cuDoubleComplex z) {
    return z.x * z.x + z.y * z.y;
}

__global__ void calculate_norms(cuDoubleComplex* v, double* norms, int N, int num_vectors) {
    /*
    ...
    */
    ...
    extern __shared__ double smem[]; // Shared memory for storing p_norm. This is shared across threads within a block. Each block gets a different shared mem.
    ...
    for (int i = bid * N + tid; i < (bid + 1) * N; i += num_threads) {
        ...
        p_norm += complex_abs_squared(val); // Accumulate the squared absolute value
    }
...
__global__ void normalize_vectors(cuDoubleComplex* v, double* norms, int N, int num_vectors) {
    /*
...
cudaError_t generateUniformRandomVectorsCuda(cuDoubleComplex *v, int N, int num_vectors) {
    ...
    Parameters:
        v: Pointer to device memory where the generated vectors will be stored. 
              The memory should be allocated with size N * num_vectors * sizeof(cuDoubleComplex).
    ...
    */
    ...
    generate_complex_normals_v<<<gridSize, blockSize>>>(d_states, v, num_vectors * N);
    cudaDeviceSynchronize();
    ...
    cudaMalloc(&d_norms, num_vectors * sizeof(double));
    ...
    size_t shared_mem_size = norm_block_size * sizeof(double);
    calculate_norms<<<norm_grid_size, norm_block_size, shared_mem_size>>>(v, d_norms, N, num_vectors);
    cudaDeviceSynchronize();

    // Normalize the vectors
    normalize_vectors<<<norm_grid_size, norm_block_size>>>(v, d_norms, N, num_vectors);
    cudaDeviceSynchronize();
