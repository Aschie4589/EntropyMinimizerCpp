...
int RandomGenerator::generate(std::vector<std::complex<double>>* kraus) {
    ...
    */
    ...
    if (err != cudaSuccess) {
        msg_handler_.error("Error generating Haar random unitaries: " + std::string(cudaGetErrorString(err)));
        return 1;
    }
    ...
    if (errcopy != cudaSuccess) {
        msg_handler_.error("Error copying Kraus operators from device to host: " + std::string(cudaGetErrorString(errcopy)));
        return 1;
    }
    msg_handler_.info("Successfully copied " + std::to_string(config.kraus_number * config.kraus_in_dimension * config.kraus_out_dimension * sizeof(cuDoubleComplex)) + " bytes or " + std::to_string(config.kraus_number * config.kraus_in_dimension * config.kraus_out_dimension * sizeof(cuDoubleComplex)/1024.0/1024/1024) + " GiB of Kraus operators from device to host.");
    // Free the device memory
    cudaError_t errfree = cudaFree(kraus_operators);
    if (errfree != cudaSuccess) {
        msg_handler_.error("Error freeing memory for Kraus operators: " + std::string(cudaGetErrorString(errfree)));
        return 1;
    }
    return 0;
