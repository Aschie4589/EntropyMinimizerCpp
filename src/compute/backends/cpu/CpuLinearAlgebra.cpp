...
int CpuLinearAlgebra::toCblasOp(Transpose trans) {
    switch (trans) {
...
void CpuLinearAlgebra::axpy(
    ...
    // Note: CPU operations ignore stream parameter (synchronous)
...
void CpuLinearAlgebra::norm2(
    ...
    if (precision == PrecisionType::DOUBLE) {
        *static_cast<double*>(result) = cblas_dznrm2(
            n,
            static_cast<const double*>(x), 1
        );
    } else {
        *static_cast<float*>(result) = cblas_scnrm2(
            n,
            static_cast<const float*>(x), 1
        );
    }
...
void CpuLinearAlgebra::gemv(
    ...
    CBLAS_TRANSPOSE op = static_cast<CBLAS_TRANSPOSE>(toCblasOp(trans));
    // lda is always the leading dimension of A in storage (number of rows)
    int lda = m;
...
void CpuLinearAlgebra::gemm(
    ...
    CBLAS_TRANSPOSE opA = static_cast<CBLAS_TRANSPOSE>(toCblasOp(transA));
    CBLAS_TRANSPOSE opB = static_cast<CBLAS_TRANSPOSE>(toCblasOp(transB));
...
void CpuLinearAlgebra::geam(
    ...
    // CBLAS doesn't have geam, so implement using element-wise operations
    // or use gemm with identity matrices
    
    // For now, implement as two separate operations:
    // C = alpha * op(A) (using copy + scale)
    // C += beta * op(B) (using gemm with identity if needed)
