...
// CUDA memory management macros
#define CUDA_MALLOC_CHECK(ptr, size, name) \
    do { \
        cudaError_t err = cudaMalloc(&(ptr), (size)); \
        if (err != cudaSuccess) { \
            std::cerr << "CUDA malloc failed for " << (name) << ": " << cudaGetErrorString(err) << std::endl; \
            std::exit(1); \
        } \
    } while(0)

#define CUDA_MEMCPY_CHECK(dst, src, size, kind, name) \
    do { \
        cudaError_t err = cudaMemcpy((dst), (src), (size), (kind)); \
        if (err != cudaSuccess) { \
            std::cerr << "CUDA memcpy failed for " << (name) << ": " << cudaGetErrorString(err) << std::endl; \
            std::exit(1); \
        } \
    } while(0)
